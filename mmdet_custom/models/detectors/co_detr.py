...
class CoDETR(BaseDetector):
    ...
    def with_roi_head(self):
        """bool: whether the detector has a RoI head"""
        return hasattr(self,
                       'roi_head') and self.roi_head is not None and len(
                           self.roi_head) > 0
    ...
    def with_shared_head(self):
        ...
        return hasattr(self,
                       'roi_head') and self.roi_head[0].with_shared_head
    ...
    def simple_test(self, img, img_metas, proposals=None, rescale=False):
        ...
        if self.with_bbox and self.eval_module == 'one-stage':
            return self.simple_test_bbox_head(img, img_metas, proposals,
                                              rescale)
        if self.with_roi_head and self.eval_module == 'two-stage':
            return self.simple_test_roi_head(img, img_metas, proposals,
                                             rescale)
        return self.simple_test_query_head(img, img_metas, proposals,
                                           rescale)
    ...
    def aug_test(self, imgs, img_metas, rescale=False):
        ...
        results_list = self.query_head.aug_test(feats,
                                                img_metas,
                                                rescale=rescale)
        bbox_results = [
            bbox2result(det_bboxes, det_labels, self.query_head.num_classes)
