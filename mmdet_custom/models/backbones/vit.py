...
class WindowedAttention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        x = F.fold(
            x, output_size=(
                H_, W_), kernel_size=(
                self.window_size, self.window_size), stride=(
                self.window_size, self.window_size))  # [B, C, H_, W_]
...
class Block(nn.Module):
    def __init__(
            ...
            with_cp=False, attn_drop=0., drop_path=0., act_layer=nn.GELU,
            norm_layer=nn.LayerNorm, windowed=False, window_size=14,
            use_residual=False, layer_scale=False):
        super().__init__()
...
class TIMMVisionTransformer(BaseModule):
    ...
    def __init__(
            self, img_size=224, patch_size=16, in_chans=3, residual_indices=[],
            embed_dim=768, depth=12, num_heads=12, mlp_ratio=4., qkv_bias=True,
            drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
            layer_scale=True, embed_layer=PatchEmbed,
            norm_layer=partial(nn.LayerNorm, eps=1e-6),
            with_cls_token=True, act_layer=nn.GELU, window_attn=False,
            window_size=14, with_cp=False, pretrained=None):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*
                                    [
                                        Block(
                                            ...
                                            with_cp=with_cp, use_residual=True
                                            if i in residual_indices else False)
                                        for i in range(depth)])

        self.init_weights(pretrained)

    def fix_keys(self, pretrained_str, state_dict):
        new_state_dict = {}
        if 'Meta-Transformer_base_patch16_encoder' in pretrained_str:
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            state_dict = self.fix_keys(pretrained_str, state_dict)
