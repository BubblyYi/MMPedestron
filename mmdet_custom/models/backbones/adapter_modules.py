...
class Extractor(nn.Module):
    def __init__(
        ...
        self.attn = MSDeformAttn(embed_dims=dim,
                                 num_levels=n_levels,
                                 num_heads=num_heads,
                                 num_points=n_points,
                                 batch_first=True)
    ...
    def forward(self, query, reference_points, feat,
                spatial_shapes, level_start_index, H, W):

        def _inner_forward(query, feat):

            attn = self.attn(query=self.query_norm(query),
...
class Injector(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, n_levels=1,
                 deform_ratio=1.0, norm_layer=partial(
...
class InteractionBlock(nn.Module):
    def __init__(
            ...
            init_values=0., deform_ratio=1.0, extra_extractor=False,
            with_cp=False):
        super().__init__()
...
class SpatialPriorModule(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384):
        ...
        self.stem = nn.Sequential(
            *
            [nn.Conv2d(
             ...
             nn.Conv2d(
                 inplanes, inplanes, kernel_size=3, stride=1, padding=1,
                 bias=False),
             nn.SyncBatchNorm(inplanes),
             nn.ReLU(inplace=True),
             nn.Conv2d(
        ...
        self.conv2 = nn.Sequential(
            *
            [nn.Conv2d(
                inplanes, 2 * inplanes, kernel_size=3, stride=2, padding=1,
                bias=False),
        ...
        self.conv3 = nn.Sequential(
            *
            [nn.Conv2d(
                2 * inplanes, 4 * inplanes, kernel_size=3, stride=2,
        ...
        self.conv4 = nn.Sequential(
            *
            [nn.Conv2d(
                4 * inplanes, 4 * inplanes, kernel_size=3, stride=2,
                padding=1, bias=False),
             nn.SyncBatchNorm(4 * inplanes),
             nn.ReLU(inplace=True)])
        self.fc1 = nn.Conv2d(
