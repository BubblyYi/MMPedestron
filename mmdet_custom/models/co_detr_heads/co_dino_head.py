...
class CoDINOHead(CoDeformDETRHead):
    ...
    def forward_aux(self, mlvl_feats, img_metas, aux_targets, head_idx):
        ...
        query_embeds = None
        hs, inter_references = self.transformer.forward_aux(mlvl_feats, mlvl_masks, query_embeds, mlvl_positional_encodings, aux_coords,
            pos_feats=aux_feats,  # noqa:E128, E281, E501
            reg_branches=self.reg_branches if self.with_box_refine else None,  # noqa:E128, E501,E281
            cls_branches=self.cls_branches if self.as_two_stage else None,  # noqa:E128, E501, E281
            return_encoder_output=True,  # noqa:E128, E281, E501
            attn_masks=attn_masks,  # noqa:E128, E281, E501
            head_idx=head_idx  # noqa:E128, E281, E501
            )  # noqa:E124
...
@HEADS.register_module()
class CoDINOCoodHead(CoDINOHead):
    def forward_train(self,
                      x,
                      img_metas,
                      gt_bboxes,
        ...
        outs = self(
            ...
            dn_bbox_query,
            attn_mask,
            cood_embed=cood_embed)
        # if cood_embed is not None:
        #     outs[3] = outs[3]+cood_embed
        #     print('cood_embed in CoDINOCoodHead:',cood_embed.size(),flush=True)
        if gt_labels is None:
            loss_inputs = outs + (gt_bboxes, img_metas, dn_meta)
        else:
            loss_inputs = outs + (gt_bboxes, gt_labels, img_metas, dn_meta)
    ...
    def forward(self,
        ...
        outputs_coords = torch.stack(outputs_coords)
        if cood_embed is not None:
            topk_anchor = topk_anchor + cood_embed

        return outputs_classes, outputs_coords, topk_score, topk_anchor, outs

    def simple_test_bboxes(self, feats, img_metas,
                           rescale=False, cood_embed=None):
        ...
        outs = self.forward(feats, img_metas, cood_embed=cood_embed)
        results_list = self.get_bboxes(
            *outs, img_metas, rescale=rescale, with_nms=with_nms)
        return results_list

    def simple_test(self, feats, img_metas, rescale=False, cood_embed=None):
        ...
        return self.simple_test_bboxes(
            feats, img_metas, rescale=rescale, cood_embed=cood_embed)
