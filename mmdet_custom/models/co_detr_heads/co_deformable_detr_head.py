...
class CoDeformDETRHead(DETRHead_Fix):
    def __init__(self,
                 *args,
                 max_pos_coords=300,
                 lambda_1=1,
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord, enc_outputs = self.transformer(
            mlvl_feats, mlvl_masks, query_embeds, mlvl_positional_encodings, reg_branches=self.reg_branches if self.with_box_refine else None, cls_branches=self.cls_branches if self.as_two_stage else None, return_encoder_output=True)
    ...
    def forward_aux(self, mlvl_feats, img_metas, aux_targets, head_idx):
        ...
        hs, init_reference, inter_references = self.transformer.forward_aux(
            mlvl_feats, mlvl_masks, query_embeds, mlvl_positional_encodings,
            aux_coords, pos_feats=aux_feats, reg_branches=self.reg_branches
            if self.with_box_refine else None, cls_branches=self.cls_branches
            if self.as_two_stage else None, return_encoder_output=True,
            attn_masks=attn_masks, head_idx=head_idx)
    ...
    def get_aux_targets(self, pos_coords, img_metas, mlvl_feats, head_idx):
        ...
        for i in range(bs):
            label = labels[i]
            feats = [feat[i].reshape(c, -1).transpose(1, 0)
                     for feat in mlvl_feats]
        ...
        for i in range(bs):
            ...
            if pos_inds.shape[0] > max_num_coords:
                indices = torch.randperm(pos_inds.shape[0])[
                    :max_num_coords].cuda()
            ...
            if self.use_zero_padding:
                ...
                attn_mask = torch.zeros(
                    [max_num_coords, max_num_coords,]).bool().to(  # noqa: E231, E261
                    coord.device)  # noqa: E231
            else:
                bbox_weights[i][:len(label)] = 1
            ...
            if coord.shape[0] < max_num_coords:
                ...
                if self.use_zero_padding:
                    ...
                    padding_label = label.new_ones(
                        [padding_shape]) * self.num_classes
                    padding_target = target.new_zeros([padding_shape, 4])
                    padding_feat = feat.new_zeros([padding_shape, c])
                    attn_mask[coord.shape[0]:, 0: coord.shape[0],] = True  # noqa: E231
                    attn_mask[:, coord.shape[0]:,] = True  # noqa: E231
                else:
                    indices = torch.randperm(neg_inds.shape[0])[
                        :padding_shape].cuda()
                    neg_inds = neg_inds[indices]
                    padding_coord = bbox_xyxy_to_cxcywh(
                        coords[i][neg_inds] / factor)
                    padding_label = labels[i][neg_inds]
                    padding_target = bbox_xyxy_to_cxcywh(
                        targets[i][neg_inds] / factor)
        ...
        if self.use_zero_padding:
            attn_masks = torch.cat(
                attn_masks, dim=0).unsqueeze(1).repeat(
                1, 8, 1, 1)
            attn_masks = attn_masks.reshape(
                bs * 8, max_num_coords, max_num_coords)
        else:
            attn_mask = None
    ...
    def _get_bboxes_single(self,
        ...
        if with_nms:
            ...
            det_bboxes, keep_idxs = batched_nms(
                det_bboxes, scores, det_labels, cfg.nms)
